...
11:30 - 12:30 | (_Lecture_) Experiments: Design and Analysis, Components of an Experiment, What is a good experiment design?
12:30 - 14:00 | Lunch break
14:00 - 15:00 | (_Hands-on_) Describe the type of data distribution of a dataset. Describe multiple hypotheses adjustment methods.
15:00 - 16:00 | (_Hands-on_) Batch effects. Confounding factors.
16:00 - 16:15 | Coffee break
16:15 - 18:00 | (_Hands-on_) Short read quality and trimming. Mapping and variant calling
...
```
curl -LO https://raw.githubusercontent.com/fpsom/CODATA-RDA-Advanced-Bioinformatics-2018/files/readCounts.csv
```
...
dataSamples <- read.csv("C:/Fotis/MyResearch/05. Workshops/CODATA-Advanced-Trieste_20_08_2018/readCounts.csv")
summary(dataSamples)
```
...
In our case, let's consider that _"Blue"_ stands for control samples, and _"Red"/"Green"_ are different treatments applied to the samples. Let's see if there is any difference in the observations of controls vs the _"Green"_ treatment.
...
![Histogram Equation](_static/images/Histogram_Equation.png "Histogram Equation")
...
We are asked to report a `p-value`. What do we do? We learned that `diff`, referred to as the _observed effect size_, is a random variable. We also learned that under the null hypothesis, the mean of the distribution of diff is 0. We use the sample standard deviation as an estimate of the population standard deviation. In R, we simply use the `sd` function and the SE is:
...
for (i in 1:B) {
  control <- sample(controlPop,N)
  se <- sd(control)/sqrt(N)
  interval <- c(mean(control)-Q*se, mean(chow)+Q*se)
  covered <- mean(controlPop) <= interval[2] & mean(controlPop) >= interval[1]
  color <- ifelse(covered,1,2)
  lines(interval, c(i,i),col=color)
}
...
Inside the `fastqc` directory you will find the report from the `fastqc` program. You can you can look at these copies of them:
...
And now view my copy of these files:
...
You can view the output html file [multiqc_report.html](http://htmlpreview.github.com/?https://github.com/ngs-docs/angus/blob/2018/_static/multiqc_report.html) by going to RStudio, selecting the file, and saying "view in Web browser."
